...
- **多阶段 RAG 链路设计与优化**
  基于 LangChain LCEL 搭建"文档解析 → 语义分块 → BGE 向量化 → 相似度检索 → LLM 生成"的 RAG 链路。引入 BM25 混合检索、关键词重排序与 Multi-Query 查询改写策略，**在 30 条 QA 评测集上达到 Optimized Recall@4 = 1.000、MRR = 0.944**。构建 30 个 QA 对评测集，从 Recall、MRR、Precision 等维度进行离线评估，基于 bad case 迭代 chunk 策略与 top-k 参数。
...
**项目成果：**
- 完成四阶段完整演进：最小 RAG → 优化检索（Recall@4 = 1.000）→ Agent 智能体 → 工程化闭环
- 解决 ChromaDB ONNX 兼容性崩溃、langchain 导入顺序冲突、RunnableParallel 线程池挂起等 6 个关键技术难题
- 构建了覆盖 24 篇 Markdown 文档、344 个文档片段的本地知识库，支持离线运行
